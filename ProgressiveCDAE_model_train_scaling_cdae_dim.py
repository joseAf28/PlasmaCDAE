...
def train_system(seed, increment_cdae):
    ...
    latent_dim = cfg.config_cade["latent_dim"] + increment_cdae
    
    hidden_dim = cfg.config_cade["hidden_dim"] + 2*increment_cdae
    ...
    map_net = models.MappingNet(input_dim=x_dim, hidden_dim=hidden_dim_map, output_dim=y_dim)
    ...
    return test_loss_map, test_refine_loss, cdae, map_net
...
if __name__ == "__main__":
    ...
    file_name = "cdae_results_scaling_cdae_dim_V2.txt"
    seed_vec = np.arange(1, 100, 20)
    hidden_dim_map_vec = np.arange(5, 60, 10)
    # print("Hidden dim map: ", hidden_dim_map_vec)
    ...
    with open(file_name, "w") as f:
        ...
        f.write("cdae dim: " + str(hidden_dim_map_vec) + "\n\n")
        
        
    for i, hidden_dim_map in enumerate(hidden_dim_map_vec):
        test_loss_map = []
        test_refine_loss = []
        for j, seed in enumerate(seed_vec):
            loss_map, loss_refine, cdae, map_net = train_system(seed, hidden_dim_map)
        ...
        with open(file_name, "a") as f:
            f.write("cdae dim: " + str(hidden_dim_map) + "\n")
